...
import { CAMPAIGNS, type CampaignFormat, type CampaignKind } from "@/lib/social/campaigns/catalog";
...
export async function POST(request: Request) {
  try {
    ...
    const asset = await renderCampaign(copy, kind, format, slug || undefined);
    return NextResponse.json({ ok: true, kind, format, slug: slug || null, ...asset, buffer: undefined, copy });
  } catch (error: any) {
